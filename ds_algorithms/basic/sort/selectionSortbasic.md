...
It requires less attempt of swaps (or memory writes) compared to many other standard ds_algorithms. Only cycle sort beats it in terms of memory writes. Therefore it can be simple algorithm choice when memory writes are costly.<br>
Disadvantages of the Selection Sort has a time complexity of O(n^2) makes it slower compared to ds_algorithms like Quick Sort or Merge Sort.<br>
...
Suitable for small lists where the overhead of more complex ds_algorithms isn’t justified and memory writing is costly as it requires less memory writes compared to other standard sorting ds_algorithms.<br>
